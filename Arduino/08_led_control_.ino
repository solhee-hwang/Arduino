...
// turn on the LED if the distance is between dist_min and dist_max
  if ((dist_raw >= dist_min && dist_raw < 130)  || (dist_raw > 270 && dist_raw <= dist_max)){
    analogWrite(PIN_LED, 240);
  }else if((dist_raw>=130&&dist_raw<150)||(dist_raw >250 && dist_raw <=270 )){
    analogWrite(PIN_LED, 180);
  }else if(dist_raw <= 250 || dist_raw >=150) {
    if(dist_raw >= 180 && dist_raw <= 220){
